...
import { analyzeLocation, type AnalysisResponse } from "@/lib/api";
        ...
        {[
          {
            ...
            d: "Your business type determines everything. We need to know exactly what you're opening.",
          },
          {
            n: "02",
...
const Index = () => {
  ...
  const selectedSubtype =
    type?.subtypes.find((s) => s.id === subtype) ?? null;
  ...
  const handleAnalyze = async () => {
    if (!pin || !type || !selectedSubtype) return;
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 3 && (
          <StepShell
            ...
            description="Sydney only for now. Continue to open the Sydney map."
            footer={navFooter(true, handleCityConfirm, () => setStep(2))}
          >
            <Step3City defaultCity="Sydney, AU" onConfirm={handleCityConfirm} />
          </StepShell>
        )}
        ...
        {step === 4 && (
          <StepShell
            ...
            footer={navFooter(!!pin && !isAnalyzing, handleAnalyze)}
          >
            <Step4Map initialCenter={center} pin={pin} onPin={setPin} />
          </StepShell>
        )}

        {step === 5 && type && subtype && pin && analysis && (
          <StepShell
            key="s5"
            stepNumber={5}
            totalSteps={6}
            ...
            description="Live market data scored against your specific concept."
        ...
        {step === 6 && type && subtype && analysis && (
          <StepShell
            ...
            description="A plain-English recommendation built from your real location analysis."
          >
            <Step6AINarrative
              type={type}
              subtypeId={subtype}
